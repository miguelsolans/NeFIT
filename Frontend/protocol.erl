...
%% enumerated types
-type 'Type'() :: 'LOGIN' | 'LOGOUT' | 'REGISTER' | 'RESPONSE' | 'ITEMORDEROFFER' | 'ITEMPRODUCTIONOFFER' | 'NOTIFICATION'.
-export_type(['Type'/0]).
...
-type 'Message'() ::
      #{user_type               => iodata(),        % = 1
        ...
        user                    => 'User'(),        % = 4
        type                    := 'LOGIN' | 'LOGOUT' | 'REGISTER' | 'RESPONSE' | 'ITEMORDEROFFER' | 'ITEMPRODUCTIONOFFER' | 'NOTIFICATION' | integer(), % = 5, enum Type
        state                   => 'State'(),       % = 6
        sale                    => 'Sale'()         % = 7
       }.
...
encode_msg_ItemOrderOffer(#{manufacturer_name := F1,
...
e_enum_Type('ITEMPRODUCTIONOFFER', Bin, _TrUserData) ->
    <<Bin/binary, 5>>;
e_enum_Type('NOTIFICATION', Bin, _TrUserData) ->
    <<Bin/binary, 6>>;
...
dfp_read_field_def_ItemOrderOffer(<<10, Rest/binary>>,
    ...
    d_field_ItemOrderOffer_manufacturer_name(Rest, Z1, Z2,
					     F@_1, F@_2, F@_3, F@_4,
					     TrUserData);
...
dfp_read_field_def_ItemOrderOffer(<<>>, 0, 0, F@_1,
				  F@_2, F@_3, F@_4, _) ->
    #{manufacturer_name => F@_1, product_name => F@_2,
...
dg_read_field_def_ItemOrderOffer(<<0:1, X:7,
    ...
    case Key of
      10 ->
	  d_field_ItemOrderOffer_manufacturer_name(Rest, 0, 0,
						   F@_1, F@_2, F@_3, F@_4,
						   TrUserData);
      18 ->
...
d_field_ItemOrderOffer_manufacturer_name(<<1:1, X:7,
					   Rest/binary>>,
					 N, Acc, F@_1, F@_2, F@_3, F@_4,
					 TrUserData)
    when N < 57 ->
    d_field_ItemOrderOffer_manufacturer_name(Rest, N + 7,
					     X bsl N + Acc, F@_1, F@_2, F@_3,
					     F@_4, TrUserData);
d_field_ItemOrderOffer_manufacturer_name(<<0:1, X:7,
					   Rest/binary>>,
...
v_msg_ItemOrderOffer(#{manufacturer_name := F1,
    ...
    lists:foreach(fun (manufacturer_name) -> ok;
		      (product_name) -> ok;
		      (quantity) -> ok;
		      (unit_price) -> ok;
		      (OtherKey) ->
...
v_msg_ItemOrderOffer(M, Path, _TrUserData)
    ...
    mk_type_error({missing_fields,
		   [manufacturer_name, product_name, quantity, unit_price]
		     -- maps:keys(M),
		   'ItemOrderOffer'},
		  M, Path);
...
get_msg_defs() ->
    [{{enum, 'Type'},
      [{'LOGIN', 0}, {'LOGOUT', 1}, {'REGISTER', 2},
       ...
       {'ITEMPRODUCTIONOFFER', 5}, {'NOTIFICATION', 6}]},
     {{msg, 'Message'},
...
enum_symbol_by_value_Type(6) -> 'NOTIFICATION'.
...
enum_value_by_symbol_Type('NOTIFICATION') -> 6.
